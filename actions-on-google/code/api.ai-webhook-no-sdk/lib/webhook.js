    ...
    intent = {
        ...
        facts: (req) => {
            return new Promise ((resolve, reject) => {
                Promise.all([data.getCatFact(), data.getCatImage()])
                    .then((data) => {
                        ...
                        if (surface.length > 1) {
                            dataResponse = {
                                ...
                                data: {
                                    google: {
                                        is_ssml: true,
                                        /*
                                         * No inputs are used if the user doesn't
                                         * try to response AFTER this fact is
                                         * served to them.
                                         */
                                        no_input_prompts: [
                                            {
                                        ...
                                        // Rich Responses are used IF on keyboard
                                        richResponse: {
                                            items: [
                                                {
                                                    simpleResponse: {
                            ...
                            dataResponse.speech = data[0];
                            dataResponse.displayText = data[0];
                        }

                        resolve(dataResponse);
                    })
                    .catch((err) => {
                        console.error(`🚒 Error intent.facts: ${JSON.stringify(err)}`);
                        reject(err);
                    });
            });
        },
